...
const translations = {
  en: {
    ...
    apiLabel: 'Backend base URL',
    apiHint: 'Required when running on GitHub Pages or other static hosts',
    saveApi: 'Save API URL',
    errors: {
  ...
  es: {
    ...
    apiLabel: 'URL base del backend',
    apiHint: 'Requerido para GitHub Pages u otros hosts estáticos',
    saveApi: 'Guardar URL de API',
    errors: {
...
function syncApiBaseInput() {
  if (!refs.apiBaseInput) return;
  refs.apiBaseInput.value = state.apiBase;
  refs.apiBaseInput.placeholder = hasConfiguredApiBase() ? getApiBase() : 'https://your-backend.example.com';
}
...
async function scrapeProduct() {
  ...
  try {
    ...
    const platform = data.platform || (isAmazon ? 'amazon' : (isAliExpress ? 'aliexpress' : 'ebay'));
    productData = { ...data, platform };
...
function handleNewChat(data, platformHint = '') {
  const platform = data.platform || platformHint || 'unknown';
  const chat = {
...
refs.apiBaseInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') saveApiBase(); });
